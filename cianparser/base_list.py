...
class BaseListPageParser:
    ...
    def print_parse_progress(self, page_number, count_of_pages, offers, ind):
        ...
        if self.count_parsed_offers == 1:
            sys.stdout.write('Parsing started')
            sys.stdout.flush()

        elif self.count_parsed_offers % 100 == 0:
            sys.stdout.write(f"\rPage: {page_number - self.start_page + 1}"
    ...
    def save_results(self):
        ...
        # Динамически определяем fieldnames на основе ключей в результатах
        fieldnames = set()
        ...
        with open(self.file_path, 'w', newline='', encoding='utf-8') as output_file:
            dict_writer = csv.DictWriter(output_file, fieldnames=fieldnames, delimiter=',', quoting=csv.QUOTE_ALL)
