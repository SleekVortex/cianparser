...
from cianparser.flat.list import FlatListPageParserAsync
...
class CianParser:
    def __init__(self, location: str, proxies=None):
        ...
        :param str location: location. e.g. "Москва", for see all correct values use list_locations()
...
def __validation_init__(location):
    ...
    if location_id is None:
        ValueError(f'You entered {location}, which is not exists in base.'
                   f' See all available values of location in list_locations()')
...
class CianParserAsync:
    def __init__(self, location: str, proxies=None):
        location_id = __validation_init__(location)
        self.__set_session__()
        self.__parser__ = None
        self.__proxy_pool__ = ProxyPoolAsync(proxies=proxies)
        self.__location_name__ = location
        self.__location_id__ = location_id
    ...
    def __set_session__(self):
        ...
        self.__session__.headers = self.headers
    ...
    def __choose_one_proxy__(self):
        proxy_current = random.choice(self.__proxy_pool__.proxy_pool)
    ...
    def __load_list_page__(self, url_list_format, page_number, attempt_number_exception):
        ...
        if page_number == self.__parser__.start_page and attempt_number_exception == 0:
            print(f"Страница, с которой начинается сбор информации: \n {url_list}")
            
        proxy = self.__choose_one_proxy__()
        res = get(url=url_list, proxy=proxy, timeout=15, headers=self.headers)

        if res.status_code == 429:
            print('429 при парсинге главной страницы, ожидание...')
            time.sleep(max(normalvariate(150, 30), 300 * expovariate(lambd=1)))
    ...
    async def __run__(self, url_list_format: str):
        ...
        print(*self.__proxy_pool__.proxy_pool, sep='\n')
        ...
        while page_number < self.__parser__.end_page and not end_all_parsing:
            ...
            while attempt_number_exception < 7 and not page_parsed:
                # Устанавливаем новую сессию
                self.__set_session__()

                try:
                    # Получаем html страницы списка объявлений
                    html = self.__load_list_page__(
                        url_list_format=url_list_format, page_number=page_number,
                        attempt_number_exception=attempt_number_exception
                    )
                    ...
                    # Парсим конкретные объявления
                    page_parsed, attempt_number_exception, end_all_parsing = await self.__parser__.parse_list_offers_page(
                        html=html,
                        page_number=page_number,
                        count_of_pages=self.__parser__.end_page + 1 - self.__parser__.start_page,
                        attempt_number=attempt_number_exception)
                    ...
                    attempt_number_exception += 1
                    if attempt_number_exception < 7:
                        continue
                    print(f"\n\nИсключение: {e}")
                    print(f"Сбор информации со страниц завершен на {page_number} странице...\n")
                    break

        print(f"\n\nСбор информации со страниц списка объявлений завершен")
        print(f"Общее количество обработанных объявлений: {self.__parser__.count_parsed_offers}. ", end="\n")


    async def get_flats(self, deal_type: str, rooms, with_saving_csv=False, with_extra_data=False, additional_settings=None):
        # Проверка прокси на доступность
        await self.__proxy_pool__.get_available_proxies()

        __validation_get_flats__(deal_type, rooms)
        deal_type, rent_period_type = __define_deal_type__(deal_type)
        self.__parser__ = FlatListPageParserAsync(
            accommodation_type="flat",
            deal_type=deal_type,
            rent_period_type=rent_period_type,
            location_name=self.__location_name__,
            ...
            with_extra_data=with_extra_data,
            additional_settings=additional_settings,
            proxies=self.__proxy_pool__.proxy_pool
        )
        await self.__run__(
            __build_url_list__(location_id=self.__location_id__, deal_type=deal_type, accommodation_type="flat",
                               rooms=rooms, rent_period_type=rent_period_type,
    ...
    async def get_suburban(self, suburban_type: str, deal_type: str, with_saving_csv=False, with_extra_data=False, additional_settings=None):
        __validation_get_suburban__(suburban_type=suburban_type, deal_type=deal_type)
        deal_type, rent_period_type = __define_deal_type__(deal_type)
        self.__parser__ = SuburbanListPageParser(
            session=self.__session__,
    ...
    async def get_newobjects(self, with_saving_csv=False):
        self.__parser__ = NewObjectListParser(
            session=self.__session__,
            location_name=self.__location_name__,
            with_saving_csv=with_saving_csv,
